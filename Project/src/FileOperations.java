...
public class FileOperations {
    ...
    public void writeHomeList(ArrayList<Home> homeList){
        ...
        try{
            ...
            for (Home home : homeList)
            {
                ...
                fileWriter.write(separator);
                fileWriter.write(home.getPrice());
                fileWriter.write(separator);
            }
        }
        catch(Exception e){
    ...
    public void readHomeList(ArrayList<Home> homeList, ArrayList<SystemUserAbstract> userList){
        ...
        int adNo = 0,  province = 1, country = 2, neighborhood = 3, status = 4, type = 5, countOfRoom = 6, contactEmail = 7, area = 8, price=9;
        BufferedReader fileReader = null;
        try{
            ...
            while((line=fileReader.readLine()) != null)
            {
                ...
                Home home = new Home(tokens[countOfRoom], tokens[status], tokens[type], address, Integer.parseInt(tokens[adNo]), user, Integer.parseInt(tokens[area]), Integer.parseInt(tokens[price]));
                homeList.add(home);
            }
